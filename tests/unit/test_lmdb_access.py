...
def shard_and_get_keys(
    lmdb_reader: lmdb_handler.LmdbAccess, shard_index: int, world_size: int, sequential: bool
) -> List[bytes]:
    keys = lmdb_reader.get_keys()
    keys = key_util.shard_keys(
...
def test_lmdb_access_keys() -> None:
    range_size = 10
    lmdb_reader = lmdb_handler.LmdbAccess(
...
def test_lmdb_access_keys_sequential_shard() -> None:
    ...
    for shard_id in range(world_size):
        lmdb_reader = lmdb_handler.LmdbAccess(lmdb_path=lmdb_checkpoint_path)
        key_shards.append(
            shard_and_get_keys(
...
def test_lmdb_access_shuffle() -> None:
    ...
    keys_one = key_util.shuffle_keys(keys=keys_one, seed=seed_one)

    lmdb_reader_two = lmdb_handler.LmdbAccess(lmdb_path=lmdb_checkpoint_path)
    keys_two = lmdb_reader_two.get_keys()
    keys_two = key_util.shuffle_keys(keys=keys_two, seed=seed_one)

    lmdb_reader_three = lmdb_handler.LmdbAccess(lmdb_path=lmdb_checkpoint_path)
    keys_three = lmdb_reader_three.get_keys()
    keys_three = key_util.shuffle_keys(keys=keys_three, seed=seed_two)
...
def test_lmdb_access_shapes_and_types() -> None:
    ...
    matching_dataset = util.create_tf_dataset_from_range(range_size=range_size)
